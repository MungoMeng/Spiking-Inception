...
To ease these limitation. We proposed a Spiking Inception architecture for unsupervised SNN. 
...
Inception module is proposed in the ANN literature. There is a Split-and-Merge strategy in the Inception module: 
...
and improve network’s parallelism. We also designed a Inception-like multi-pathway network architecture. 
To further improve the architecture’s learning efficiency and robustness, we divided each pathway into multiple parallel 
sub-networks by partitioning competition areas. Finally we attained a high-parallelism Inception-like netwrok architecture
consisting of 21 parallel sub-networks.   

## Instruction
Here we provide a implementation of the unsupervised SNN with our spiking Inception architecture. All code is written in **Python2**.
...
and set the `MNIST_data_path` in the `Functions.py` to specify the the directory containing the data. 
...
Then, you can test the trained SNN with a simple commond:  
```
python Test.py
```
Note that runing `Test.py` won't give you a testing result (accuracy) directly. 
It only saves the spiking activity in the a directory named `activity` (Sometimes you need to create it manually)..

### Evaluation
You can use the folowing command to get a testing result (accuracy) on the testing set of MNIST.
```
